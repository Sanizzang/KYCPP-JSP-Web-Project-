...
<body>
	<%
		...
		int result = userDAO.update(multi.getParameter("name"), multi.getParameter("major"), multi.getParameter("studentId"), multi.getParameter("gender"), multi.getParameter("biography"), original, email);
		if(result == -1){
			PrintWriter script = response.getWriter();
			...
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alert('프로필을 수정하였습니다.')");
			script.println("location.href = 'index.jsp'");
			script.println("</script>");
		}
	%>
